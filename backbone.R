#Script to create an integrated GBIF-NCBI taxonomic backbone, including intermediate stage outputs.
rm(list=ls())
require(dplyr)
...
custom_taxonomy <- fread(input=paste(taxonomy_home,"ncbi_gbif_backbone_full.tsv",sep="/"),sep="\t")
...
#Save filtered NCBI and GBIF taxonomies.
write.table(filtered_taxonomy, paste(taxonomy_home,"ncbi_gbif_backbone_filtered.tsv",sep="/"), sep = "\t", col.names = TRUE, row.names = FALSE)
filtered_taxonomy <- fread(input=paste(taxonomy_home,"ncbi_gbif_backbone_filtered.tsv",sep="/"),sep="\t")
...
OTL_taxonomy <- fread(input=paste(taxonomy_home,"otl_taxonomy.tsv",sep="/"),sep="\t")
...
#Save linked NCBI and GBIF taxonomies from OTL.
write.table(OTL_GBIF_NCBI, paste(taxonomy_home,"OTL_GBIF_NCBI.tsv",sep="/"), sep = "\t", col.names = TRUE, row.names = FALSE)
OTL_GBIF_NCBI <- fread(input=paste(taxonomy_home,"OTL_GBIF_NCBI.tsv",sep="/"),sep="\t")
OTL_GBIF_NCBI <- as.data.frame(OTL_GBIF_NCBI)
...
ncbi_to_add <- dplyr::left_join(ncbi_to_add,OTL_GBIF_NCBI,multiple="all")
...
#Save table.
write.table(filtered_taxonomy_expanded, paste(taxonomy_home,"ncbi_gbif_backbone_filtered_expanded.tsv",sep="/"), sep = "\t", col.names = TRUE, row.names = FALSE)

#Create taxonomic keys for GBIF taxonomies.
filtered_taxonomy_expanded <- fread(input=paste(taxonomy_home,"ncbi_gbif_backbone_filtered_expanded.tsv",sep="/"),sep="\t")
filtered_taxonomy_expanded <- as.data.frame(filtered_taxonomy_expanded)
...
for(taxonomicRank in taxonomicRanks){
  ...
  print("Preferentially retain taxa with accepted taxonomies.")
  ...
  print("Group by taxonID and taxon, count occurrences.")
  grouped <- taxon_tmp %>% group_by(!!sym(taxonomicRank),taxonID) %>% summarize(count = n())
  # Find the most common entry for each 'taxonID'
  print("Find the most common entry for each taxonID")
  most_common <- grouped %>% group_by(taxonID) %>% slice_max(order_by = count, n = 1) %>% ungroup()
  #Keep only the first entry for each 'taxonID'
  print("Keep only the first entry for each taxonID")
  ...
  print(paste(taxonomicRank,"completed"))
}
#Save table
write.table(filtered_taxonomy_withKeys, paste(taxonomy_home,"ncbi_gbif_backbone_filtered_expanded_withKeys.tsv",sep="/"), sep = "\t", col.names = TRUE, row.names = FALSE)

#Get the most common English common names per GBIF taxon ID
Vernacular_Input <- fread(input=paste(taxonomy_home,"VernacularName.tsv",sep="/"),sep="\t")
Vernacular_Input <- as.data.frame(Vernacular_Input)
...
first_common$taxonID <- as.numeric(first_common$taxonID)
#Export table
write.table(first_common, paste(taxonomy_home,"VernacularName_MostCommon.tsv",sep="/"), sep = "\t", col.names = TRUE, row.names = FALSE)

#Get available common names for all taxa and ranks. Merge in common names.
first_common <- fread(input=paste(taxonomy_home,"VernacularName_MostCommon.tsv",sep="/"),sep="\t")
first_common <- as.data.frame(first_common)
filtered_taxonomy_withKeys <- fread(input=paste(taxonomy_home,"ncbi_gbif_backbone_filtered_expanded_withKeys.tsv",sep="/"),sep="\t")
filtered_taxonomy_withKeys <- as.data.frame(filtered_taxonomy_withKeys)
...
#Export table
write.table(filtered_taxonomy_withNames, paste(taxonomy_home,"ncbi_gbif_backbone_filtered_expanded_withCommonNames.tsv",sep="/"), sep = "\t", col.names = TRUE, row.names = FALSE)

#Retain certain columns and then remove duplicate rows.
filtered_taxonomy_withNames <- fread(input=paste(taxonomy_home,"ncbi_gbif_backbone_filtered_expanded_withCommonNames.tsv",sep="/"),sep="\t")
filtered_taxonomy_withNames <- as.data.frame(filtered_taxonomy_withNames)
...
taxonomy_export <- taxonomy_export %>%
  ...
  select(-count)
#Export NCBI-GBIF backbone.
write.table(taxonomy_export, paste(taxonomy_home,"GBIF_NCBI.csv",sep="/"), sep = "\t", col.names = TRUE, row.names = FALSE)
