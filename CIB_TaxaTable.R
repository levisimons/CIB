...
for(Project_Directory in Project_Directories){
  ...
  MetadataTable <- dplyr::left_join(MetadataTable, Metadata_Processed[,c("name","Sample_Date")],by=c("Sample ID"="name"))
  ...
  if(file.exists(TronkoTable_Path)){
    ...
    if(nrow(tax_mat)>1){
      ...
      Project_Samples_Filtered <- nrow(Metadata_Filtered)
      #
...
m
#
taxa <- colnames(MergedData[,6:ncol(MergedData)])
n <- 988
taxon <- taxa[n]
